...
image_path = "/Users/tina/Downloads/wikiart/Abstract_Expressionism/aaron-siskind_acolman-1-1955.jpg"
#image_path="img.jpg"
...
paint_emotion_dict = {}
#dictionary that the keys are image paths and the values are emotions
...
df2 = pd.DataFrame()
#dataframe included image paths and image emotions
paths = []
labels = []
for k,v in paint_emotion_dict.items():
    paths.append(k)
    labels.append(sorted(list(set(v)))) #we sort emotions by alphabet
...
# Preprocess the dataset to remove rows with non-existing image paths --- using our previous function
preprocessed_df = preprocess_dataset(df2)
print(preprocessed_df.head())
#print(preprocessed_df.head(3))


# Define your transformations ------------------------------------------------------ nafahmidam =((((
...
NUM_CLASSES = len(df['emotion_label'].unique()) #for computing the number of possible emotions

from sklearn.preprocessing import MultiLabelBinarizer
import random

class EmotionDataset(Dataset):
    def __init__(self, df, transform=None, subset_percentage=0.2):      #defunt of transformer is none
        self.df = df.sample(frac=subset_percentage, random_state=42)  # Randomly sample a subset of data
        self.transform = transform
        self.mlb = MultiLabelBinarizer()       #convert multi-label data into binary format
        self.labels = self.mlb.fit_transform(self.df.emotions)    #binary table of amotions unsing 0 and 1

    def __len__(self):
        return len(self.df)  #return the umber of rows in dataframe

    def __getitem__(self, idx):
        row = self.df.iloc[idx]
        image = Image.open(row['image_file']).convert('RGB')
        if self.transform:            #------------------------------------------------------ nafahmidam =((((
...
# Create dataloader
train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True)   #object from dataloader of pytorch
validation_loader = DataLoader(validation_dataset, batch_size=BATCH_SIZE, shuffle=False)  #object from dataloader class of pytorch

# Load pre-trained ResNet model
model = models.resnet50(pretrained=True)

#Unfreeze the model (we want to change pretrained parameters)
for param in model.parameters():
    param.requires_grad = True

# Replace the last layers
num_ftrs = model.fc.in_features        #number of input features to the final fully connected 
model.fc = nn.Sequential(
    
    #layer1
    nn.Linear(num_ftrs, 768),          #argumnets(numberof layer inputs,number of layer outputs)
    nn.ReLU(),
    nn.BatchNorm1d(768),               #Normalizes the activations to have a mean of 0 and a variance of 1
    nn.Dropout(0.5),
...
def train(model, train_loader, val_loader, criterion, optimizer, device, num_epochs, csv_file, history):
    since = time.time()    #seconds since January 1, 1970
    best_val_acc = 0.0
    for epoch in range(num_epochs):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):   #(data, labels) ---- batch index starts from zeo and its a counter
            images = images.to(device)
            labels = labels.to(device)

            optimizer.zero_grad()    #Reset gradients to zero before calculating new gradients
            outputs = model(images)  #list of probabilities of output layer
            #print(outputs)
            
            _, predicted = torch.max(outputs.data, 1)       #------------------------why we have two predicted parts?
            predicted = (outputs > 0.5).float()             #output is a list including 0,1 ----> 1 for each probability grater than 0.5
            
            # predicted = F.one_hot(predicted, num_classes=NUM_CLASSES).float()
            # if(batch_idx % 10 == 0):
            #   print(f'{labels} \n {outputs}')
            
            loss = criterion(outputs, labels)    #used to calculate the loss value between the model's (outputs) and the actual target values (labels)
            loss.backward()                      #backpropagation
            optimizer.step()                     #updates the model parameters using the gradients

            # Convert predicted and labels tensors to NumPy arrays
            predicted_numpy = predicted.cpu().numpy()     #Moves the tensor to the CPU--if it is on GPU
            labels_numpy = labels.cpu().numpy()


            train_loss += loss.item() * images.size(0)                            #image.sie(0)= number of samples ----> get the total loss for this batch
            train_correct += accuracy_score_cal(labels_numpy, predicted_numpy)
            train_total += labels.size(0)
            train_total_acc += 64*9          #why this number?
            # print(train_correct)
            # print(train_total)
            
            #convert tensor to list - Adding
            train_predictions.extend(predicted.tolist())
            train_targets.extend(labels.tolist())


            # Calculate additional evaluation metrics
            train_hamming_loss = hamming_loss(train_targets, train_predictions)      #Hamming=(Number of incorrect labels/total number of labels)
